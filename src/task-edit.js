    ...
    return {
      ...
      date: (value) => {
        if (moment(value).isValid()) {
          target.dueDate = moment(value + ` ` + moment(target.dueDate).format(`h:mm A`), `DD MMMM h:mm A`).toDate();
        }
      },
      time: (value) => {
        if (value) {
          target.dueDate = moment(moment(target.dueDate).format(`DD MMMM`) + ` ` + value, `DD MMMM h:mm A`).toDate();
        }
      }
    };
  }
